...
async function main() {
  ...
  let balanceGold = await tokenERC1155.balanceOf(wallet.address, 0);
  let balanceNFT = await tokenERC1155.balanceOf(wallet.address, 1);
  ...
  await tokenERC1155.mint(wallet.address, 0, 200, "0x00");
  await tokenERC1155.mint(wallet.address, 1, 1, "0x00");

  // balanceA = await tokenA.balanceOf(wallet.address);
  // balanceB = await tokenB.balanceOf(wallet.address);
  balanceGold = await tokenERC1155.balanceOf(wallet.address, 0);
  balanceNFT = await tokenERC1155.balanceOf(wallet.address, 1);
